...
class RentYourSpaceViewController: BaseViewController  {
    ...
    override func viewDidLoad() {
        ...
        self.lblHeader.text = "Add Listing"//"RENT YOUR " + getCategory()
        ...
        //            let parameterStr = "currency_code=\(login_session.value(forKey: "APP_CURRENCY") as! String)&lang_code=\(lanuguage_selection.value(forKey: "language") ?? "en")&base_id=\(id)"
        //            Network.shared.POSTRequest(withParameterString:parameterStr , serviceURL: REQ_RENT_YOUR_SPACE, APIKEY: "RENTYOURSPACE")
        //            print(parameterStr)
        //        } else {
...
//        if choose == "en"
//        {
            // self.lblHeader.text = LocalizeHelper().localizedStringForKey("rentyourspace1")
//            self.txtCity.placeholder = LocalizeHelper().localizedStringForKey("city")
//            self.txtRoomType.placeholder = LocalizeHelper().localizedStringForKey("roomtype")
            self.txtAccommodates.placeholder = "User Capacity"
//            self.txtPropertyType.placeholder = LocalizeHelper().localizedStringForKey("propertytype")
            self.roomType = LocalizeHelper().localizedStringForKey("roomtype")
            self.accomodates = LocalizeHelper().localizedStringForKey("accomodateserror")
            self.propertyType = LocalizeHelper().localizedStringForKey("propertytype")
//        }
    }
...
//        self.navigationController?.popViewController(animated: true)
    ...
    @IBAction func act_Done(_ sender: UIButton) {
        ...
        if self.room_ID == ""  || txtPropertyType.text?.count == 0 {
            showInformation(title: "Closest", message: "Please fill all details")
...
//                        UserDefaults.standard.set(responseDict.value(forKey: "attribute"), forKey: "AmenitiesArray")
                ...
                //                APIManager.apiPost(serviceName: REQ_RENT_YOUR_SPACE_STEP1, parameters: params) { (json:NSDictionary?, error:NSError?) in
                ...
                //
                //                    let vc = self.storyboard?.instantiateViewController(withIdentifier: "ListingViewControllersBundle") as! ListingViewControllersBundle
                //                    Singleton.sharedInstance.rentYourSpace = RentYourSpaceModel(fromDictionary: json as! [String : Any])
                //                    checkedIDArray.removeAllObjects()
                //                    UserDefaults.standard.set(json!.value(forKey: "attribute"), forKey: "AmenitiesArray")
                //
                //                    commonMaintainListDataDict.removeAllObjects()
                //                    commonMaintainListDataDict.addEntries(from: json as! [AnyHashable : Any])
                //                    print("commonMaintainListDataDict count:::",commonMaintainListDataDict.count)
                //
                //                    forAddPhotoDict.removeAllObjects()
                //                    forAddPhotoDict.addEntries(from: result[0] as! [AnyHashable : Any])
                ...
                //                                let parameterString =  Network.shared.POSTRequest(withParameterString: parameterString , serviceURL: REQ_RENT_YOUR_SPACE_STEP1, APIKEY: "SAVE_RENTYOURSPACE")
                //                            } "user_id=\(userid)&property_type=\(self.prop_ID)&room_type=\(self.room_ID)&accommodates=\(self.txtAccommodates.text!)&city=\(self.txtCity.text!)&lang_code=\(lanuguage_selection.value(forKey: "language") ?? "en")&base_id=\(id)"
                //                                print(id)
                ...
                self.showInformation(title: "Info", message: "Please connect to internet")
            }
        }
    }
