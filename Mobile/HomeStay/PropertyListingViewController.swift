...
//
////            let userID = login_session.value(forKey: "UserId")!
////            let  parameterDict = "userid=\(userID)&currency_code=\(login_session.value(forKey: "APP_CURRENCY") as! String)&lang_code=\(login_session.value(forKey: "APP_LANGUAGE") as! String)&base_id=\(Singleton.sharedInstance.selectedCategory!)"
////            print(parameterDict)
//            //            Network as! (DataResponse<Any>) -> Void.shared.POSTRequest(withParameterString: parameterDict, serviceURL: SHOW_PROPERTY_LISTING, APIKEY: "SHOW_PROPERTY_LISTING")
//        }
//        else
//        {
//            showInformation(title: "Rental Product", message: "Please check your internet connection.")
//        }
//
...
//
//        params = ["userid": userID,"currency_code":login_session.value(forKey: "APP_CURRENCY") as! String,"lang_code":login_session.value(forKey: "APP_LANGUAGE") as? String ?? "en","base_id":1]
//
//        let manager = AFHTTPSessionManager()
//        manager.responseSerializer.acceptableContentTypes = NSSet(array: ["text/plain", "text/html", "application/json"]) as Set<NSObject> as? Set<String>
//
//        manager.post(SHOW_PROPERTY_LISTING, parameters: params, progress: nil, success: { (operation, responseObject) -> Void in
...
//            self.viewMoveableBar.frame = CGRect(x: 0, y: self.viewMoveableBar.frame.origin.y, width: self.view.frame.width / 2 , height: 3)
//        })
//
//        if Singleton.sharedInstance.PropertyListingModel != nil
//        {
//
//        if Singleton.sharedInstance.PropertyListingModel.propertyListing.count == 0 {
//            self.lblNoDatasFound.isHidden = false
...
//    {
//        if isFromPaymentSuccess == true
//        {
//            isFromPaymentSuccess = false
...
//
//            if dict.propertyTitle! == ""
//            {
//                cell!.lbl_Name.text = "Untitled"
//            }
//            else
//            {
//            cell!.lbl_Name.text = dict.propertyTitle!
//            }
//            cell!.lbl_Status.text = manageCalender
...
//            cell!.lblPaymentStatus.attributedText = NSAttributedString().changeColor(text1: housePayment + " : ", text2: "\(dict.paymentStatus!)", color1: UIColor.black, color2: UIColor.red)
//            cell!.lblHostApproval.attributedText = NSAttributedString().changeColor(text1: houseApproval + " : ", text2: "\(dict.approvalStatus!)", color1: UIColor.black, color2: UIColor.red)
...
class PropertyListingViewController: BaseViewController {
    ...
    func setData ()
    {
        if Reachability()!.isReachable {
            ...
            showInformation(title: "Warning", message: "Please check your internet connection.")
        }
    }
    ...
    func propertyListing(){
        self.showActivityIndicator(uiView: self.view)
//                  let userID = login_session.value(forKey: "UserId")!
//        let Email = login_session.value(forKey: "Email")!
                var params = NSMutableDictionary()
...
//                manager.responseSerializer.acceptableContentTypes = NSSet(array: ["text/plain", "text/html", "application/json"]) as Set<NSObject> as? Set<String>
//
//        manager.post(SHOW_PROPERTY_LISTING, parameters: params, headers: ["Authorization":""], progress: nil, success: { (operation, responseObject) -> Void in
//                    DispatchQueue.main.async {
//        self.hideActivityIndicator(uiView: self.view)
//                    }
...
//        })
    ...
    @IBAction func act_Back(_ sender: UIButton) {
//        let nextViewController = storyBoardHome.instantiateViewController(withIdentifier: "HomeViewController") as! HomeViewController
...
//                self.showActivityIndicator(uiView: self.view)
//
//                let parametrStr = "email=\(login_session.value(forKey: "Email") as! String)&prd_id=\(Singleton.sharedInstance.PropertyListingModel.myReservation[selectedInt].propertyId!)&Bookingno=\(Singleton.sharedInstance.PropertyListingModel.myReservation[selectedInt].bookingno!)&cancellation_percentage=\("0")&user_id=\(Singleton.sharedInstance.PropertyListingModel.myReservation[selectedInt].guestId!)&disputer_id=\(login_session.value(forKey: "UserId")!)&message=\(self.cancelMsgTxtFld.text!)"
//                Network.shared.POSTRequest(withParameterString: parametrStr, serviceURL: HOST_CANCEL_BOOKING, APIKEY: "HOST_CANCEL_BOOKING")
//
//                print(parametrStr)
            }
        }
...
//                cell!.lbl_statusfield.font = UIFont(name: SemiBoldFont, size: 14)
                ...
                if dict.propertyStatus! == true {
                    cell?.lbl_statusfield.text = "Published"
                }else{
                    cell?.lbl_statusfield.text = "Waiting for publish"
                }
...
//                    cell!.lbl_PayOrManage.cornerRadius = 2
//                    cell!.lbl_PayOrManage.frame.size = CGSize(width: 10, height: 20)
//                    cell?.lbl_PayOrManage = CGSize(width: 60, height: 20)
                } else {
...
//                cell?.EmailIdVerifyBtn.tag = indexPath.row
//                cell?.HostBtn.tag = indexPath.row

//                cell?.ManageLsitBtn.tag = indexPath.row

//                cell!.EmailIdVerifyBtn.addTarget(self, action: #selector(emailIdBtnAct(_:)), for: .touchUpInside)
//                cell!.HostBtn.addTarget(self, action: #selector(HostBtnAct(_:)), for: .touchUpInside)
//                cell!.ManageLsitBtn.addTarget(self, action: #selector(ManageListBtnAct(_:)), for: .touchUpInside)
...
//                                self.subTotalLbl.text = String(format: "Booked for %@ Days : %@",hotelDict!.value(forKey: "numofdates") as! CVarArg,hotelDict!.value(forKey: "TotalwithoutService") as! CVarArg )
    ...
    //                nav!.paypal_status = self.Paypal_Status
    //
    //                self.navigationController?.pushViewController(nav!, animated: true)
                }
                else {
