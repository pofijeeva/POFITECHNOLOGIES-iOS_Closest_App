...
//            self.profileImageView.contentMode = .scaleAspectFit
    ...
    func Trust_Verification(){
        ...
        APIManager.apiPostWithHeaders(serviceName: TRUST_VERIFY_API, parameters: parameters) { (json:NSDictionary?, error:NSError?) in
            ...
            if responseDict.value(forKey: "code") as! NSNumber == 200 {
              ...
              //  self.verifiedType = verifyDict["u_idproof_type"] as! String
    ...
    @IBAction func Act_SubmitOTP(_ sender: Any) {
        if self.TVlblPhoneNumber.text == "Phone Number*" ||  self.TVlblPhoneNumber.text == "OTP *" {
            let Text = self.TVPhoneNumber.setFlag(key: .IN)
            let Te = self.TVPhoneNumber.text!
           let Xe = Te.replacingOccurrences(of: " ", with: "")
            let Ph = "\(Xe)"
            let title = (sender as AnyObject).titleLabel?.text ?? ""
            if title == "SUBMIT" {
                // verify api
...
//                self.ListingActivityDelegate?.hideActivity()
            ...
            if error != nil {
                ...
                let passAlert = UIAlertController(title: "Closest", message: "Please Change the Password", preferredStyle: UIAlertControllerStyle.alert)

                                passAlert.addAction(UIAlertAction(title: "Ok", style: .default, handler: { [self] (action: UIAlertAction!) in
...
//                self.showInformation(title: "Closest", message: error!.localizedDescription)
                ...
                if responseDict.value(forKey: "code") as! Int == 200 {
                    ...
                    let passwordAlert = UIAlertController(title: "Closest", message: "Password changed successfully!", preferredStyle: UIAlertControllerStyle.alert)

                                    passwordAlert.addAction(UIAlertAction(title: "Ok", style: .default, handler: { [self] (action: UIAlertAction!) in
...
//                sharedInstanceListing.gotoStepSeven()
    ...
    func showSimpleAlert() {
        let alert = UIAlertController(title: "HomeStay", message: "Are you sure want to login ?",         preferredStyle: UIAlertControllerStyle.alert)
...
//        cell.SNoLbl.text = "1"
        ...
        if "\(idVerifyStatus)" == "0" {
            ...
            cell.ProofStatusLbl.text = "\(idVerifyStatus)"
