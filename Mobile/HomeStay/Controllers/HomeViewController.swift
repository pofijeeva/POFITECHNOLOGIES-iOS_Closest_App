...
class HomeViewController: BaseViewController, UITextFieldDelegate,delegateForHomeFeatured, delegateForHomeProperty,FSPagerViewDataSource,FSPagerViewDelegate {
    ...
    @IBOutlet weak var BannerView: FSPagerView!{
        didSet {
            let nib = UINib(nibName: "PagerCollectionViewCell", bundle: nil)
            self.BannerView.register(nib,forCellWithReuseIdentifier: "PagerCollectionViewCell")
        }
    }
...
//                    }
                ...
                if indexPath.section == 0 {
                    ...
                    cell.CheckInLbl.text = GlobalLanguageDictionary.object(forKey: "Key_location") as? String
                    cell.CheckOutLbl.text = GlobalLanguageDictionary.object(forKey: "Key_checkIndate") as? String
                    
                    cell.LocationLbl.text = GlobalLanguageDictionary.object(forKey: "Key_checkOutdate") as? String
                    ...
                    cell.CheckInBtn.contentHorizontalAlignment = .left
                    cell.PropertTypeBtn.contentHorizontalAlignment = .left
                    cell.CheckOutBtn.contentHorizontalAlignment = .left
                    if let select = GlobalLanguageDictionary.object(forKey: "key_select") as? String {
                        if checkinDate.isEmpty {
                            cell.CheckOutBtn.setTitle(HomeStayConstants.kEmptySpace + select, for: .normal)
                        } else {
                            cell.CheckOutBtn.setTitle(checkinDate, for: .normal)
                        }
                        if CheckOutDate.isEmpty {
                            cell.LocationBtn.setTitle(HomeStayConstants.kEmptySpace + select, for: .normal)
                        } else {
                            cell.LocationBtn.setTitle(CheckOutDate, for: .normal)
                        }
                        if LocationName.isEmpty {
                            cell.CheckInBtn.setTitle(HomeStayConstants.kEmptySpace + select, for: .normal)
                        } else {
                            cell.CheckInBtn.setTitle(LocationName, for: .normal)
                        }
                        if PropertyTypeName.isEmpty {
                            cell.PropertTypeBtn.setTitle(HomeStayConstants.kEmptySpace + select, for: .normal)
                        } else {
                            cell.PropertTypeBtn.setTitle(PropertyTypeName, for: .normal)
                        }
                    }
                    cell.CheckInBtn.addTarget(self, action: #selector(LocationBtnTaped), for: .touchUpInside)
                    cell.CheckOutBtn.addTarget(self, action: #selector(CheckInBtnTaped), for: .touchUpInside)
                    cell.LocationBtn.addTarget(self, action: #selector(CheckOutBtnTaped), for: .touchUpInside)
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if tableView == self.tableViewSideMenu {
            ...
            if indexPath.section == 1 {
                if self.featuredCityList.count == 0 {
                    return 0
                } else {
                    return 190
                }
