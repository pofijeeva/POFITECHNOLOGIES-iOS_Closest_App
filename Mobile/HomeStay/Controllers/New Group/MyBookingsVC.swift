...
//        self.navigationController?.popViewController(animated: true)
    ...
    @objc func CancelButtonTapped(sender: UIButton) {
        ...
        if (Reachability()?.isReachable)! {
            ...
            let parameters:[String : Any] =  ["lang_code" : lanuguage_selection.value(forKey: "language") ?? "en",
                       "property_id": model["property_id"] as? Int ?? 0,
                       "cancel_booking_id": model["booking_id"] as AnyObject,
                       "cancel_check_in": model["check_in"] as AnyObject,
        ...
        print(parameters)
            ...
            APIManager.apiPostWithHeadersCodable(serviceName: GUESTCANCEL, parameters:parameters) { (data, error) in
                self.hideActivityIndicator(uiView: self.view)
                guard let data = data else {
...
//                        self.tblView.reloadData()
                        ...
                        self.getBookingApiResponse(params: parameter)
...
//            }
            ...
            if "\(bookingsObj["cancel_sts"] as AnyObject)" == "0" {
            ...
            cell.btn3.addTarget(self, action: #selector(CancelButtonTapped(sender:)), for: .touchUpInside)
