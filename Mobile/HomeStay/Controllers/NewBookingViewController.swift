        ...
        func instantPayAPICall() {
            ...
            if Reachability()!.isReachable {
                ...
                else
                {
                    ...
                    let params = ["check_in":self.checkinDate,"check_out":self.checkoutDate,"currency_code":login_session.value(forKey: "APP_CURRENCY") as! String,"property_id":self.PropertyId,"type":"2","lang_code":"en","add_on_total":self.AddonsAmount,"add_on_detail":AddonString] as [String : Any]
                    
                     
             
                    print(params)
                    APIManager.apiPostWithHeaders(serviceName: HOST_REQ_INSTANT_PAY_API, parameters: params) { (json:NSDictionary?, error:NSError?) in
                        DispatchQueue.main.async {  }
                        if error != nil {
                            print(error!.localizedDescription)
                            self.showInformation(title: "Closest", message: error!.localizedDescription)
...
//
//
//                    params = ["base_id":id,"P_Id":Singleton.sharedInstance.PropertyDetail.rentalId!,"s_date":self.checkinDate,"e_date":self.checkoutDate,"currency_code":login_session.value(forKey: "APP_CURRENCY") as! String,"numofdates":self.numDates,"user_id":login_session.value(forKey:"UserId")!,"renter_id":Singleton.sharedInstance.PropertyDetail.hostId!,"totalAmt":self.TotalAmount,"guests":self.GuestCnt,"cancel_percentage":self.cancel_percentage,"secDeposit":self.SecurityDeposit,"serviceFee":self.ServiceFee,"subTotal":self.SubTotal,"email":login_session.value(forKey: "Email") as! String,"message":message,"property_currency_code":login_session.value(forKey: "APP_CURRENCY") as! String,"booking_type" : self.pricetype,"price" : Singleton.sharedInstance.PropertyDetail.productprice!,"no_of_weeks" : self.no_of_weeks,"no_of_hours" : self.no_of_hours,"no_of_days" : self.no_of_days,"no_of_months" : self.no_of_months,"numofdates"  : self.no_of_days,"device_type":"ios","bundle_checkindate":bundle_checkindate,"bundle_checkoutdate":bundle_checkoutdate,"bundle_amount":Bundle_amount ,"choosed_option":"","bundle_details":BundleString,"add_on_amount":add_on_amount,"add_on_details":AddonString,"choosed_option":PaymentType,"pay_balance":self.pay_balance]
...
//                             let PaypalStatus = responseDict.value(forKey: "paypal_status") as! String
//                             let CreditCardStatus = responseDict.value(forKey: "Credit_Card_status") as! String
//                             let Enquiry = responseDict.value(forKey: "enquiryId") as AnyObject
//
//                            if StripeStatus == "Enable" {
//                                PaymentMethodsArr.add("Stripe")
//                            }else{
...
//                            nav!.choosed_option = self.PaymentType
//                            nav!.pay_balance = self.pay_balance
//                            nav!.PropertId = "\(Singleton.sharedInstance.PropertyDetail.rentalId!)"
//                            self.navigationController?.pushViewController(nav!, animated: true)
//
//                        }
//                        else {
...
//                      self.SendMessage.isHidden = false
    ...
    func HourCalcAPI()
    {
        ...
        print(BundleString)
        print(AddonString)
        let params = ["pickup": self.hourPick,"vehicle_id": self.PropertyId,"currency_code": login_session.value(forKey: "APP_CURRENCY") as? String ?? "USD","drop": self.hourDrop,"DiffHrs":self.diffHrs,"booking_type" : "hourly" ,"device_type":"ios","bundle_checkindate":bundle_checkindate,"bundle_checkoutdate":bundle_checkoutdate,"bundle_amount":Bundle_amount ,"choosed_option":"","bundle_details":BundleString,"add_on_amount":add_on_amount,"add_on_details":AddonString] as [String : Any]
        print(params)
...
extension NewBookingViewController: UITableViewDelegate,UITableViewDataSource {
    ...
    @objc func PlusbtnClick(sender: UIButton)
    {
       // print(Int(Singleton.sharedInstance.PropertyDetail.guest_capacity!))
        ...
        if self.GuestCnt ==  Int("\((self.ProperdyDetails.object(forKey: "property_details") as! NSDictionary).object(forKey: "max_qty") as AnyObject)") {
            showInformation(title: "Closest", message: "Guest capacity can't be morethan \("\((self.ProperdyDetails.object(forKey: "property_details") as! NSDictionary).object(forKey: "max_qty") as AnyObject)")")
        }else{
            self.GuestCnt += 1
            DayCalcAPI()
                   self.BookingTable.reloadData()
    ...
    {
        if self.GuestCnt == 1 {
             showInformation(title: "Closest", message: "Guest count can't be lessthan one")
        }else{
            self.GuestCnt -= 1
            DayCalcAPI()
                   self.BookingTable.reloadData()
...
//            })
    ...
    func sambagTimePickerDidSet(_ viewController: SambagTimePickerViewController, result: SambagTimePickerResult) {
        ...
        if isSelectTime == "PickUpTime"
        {
                self.hourPick = self.hPickDate + " " + "\(result)"
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            if indexPath.row == 0 {
                ...
                cell!.ProductName.text = "DESCRIPTION"
              ...
              //  cell!.ProductPrice.text = "\(currencySymbol) \(Singleton.sharedInstance.PropertyDetail.productprice!) day \(currencySymbol) \(Singleton.sharedInstance.PropertyDetail.rental_hourly_price!) hr"
                
//                if "\(Singleton.sharedInstance.PropertyDetail.productprice!)" == "0.00"
//                {
//                    cell!.ProductPrice.text = "Price : \(currencySymbol) \(Singleton.sharedInstance.PropertyDetail.rental_hourly_price!)/ Hr"
//
//                }
//                if "\(Singleton.sharedInstance.PropertyDetail.rental_hourly_price!)" == "0.00" {
//                    cell!.ProductPrice.text = "Price : \(currencySymbol) \(Singleton.sharedInstance.PropertyDetail.productprice!)/ Day"
//                }
              ...
              //  let imgUpdated =  URL(string:Singleton.sharedInstance.PropertyDetail.productImage[0].productImage!)
                ...
                //"\(Singleton.sharedInstance.PropertyDetail.productprice!)" "\(Singleton.sharedInstance.PropertyDetail.weekPrice!)"
              ...
              //  cell!.HourlyPriceValue.text = "\(currencySymbol) \(Singleton.sharedInstance.PropertyDetail.min_hour_exprice!)"
...
//                                       cell!.DayLbl.textColor = UIColor.white
        ...
        else
        {
            ...
            cell!.addonsValueLbl.text = "\(self.currencySymbol) \(self.AddonsAmount)"
            cell!.bundleValueLbl.text = "\(self.currencySymbol) \(self.AddonsAmount)"

            
            cell!.BookingLbl.text = "Property Amount ( \(self.GuestCnt) * Guests)"
